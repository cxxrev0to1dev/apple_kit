...
namespace activation_win{
    namespace internal{
		...
		const wchar_t* user_agent = L"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2587.3 Safari/537.36";
		std::string ReadHTTPS(const wchar_t* domain,const wchar_t* path,const wchar_t* header,const wchar_t* referer,const char* port) {
			HINTERNET hOpen = 0;
		...
		std::string SendHTTPS(const wchar_t* domain,const wchar_t* path,const void* src,const size_t length,const wchar_t* header,const wchar_t* referer,const char* post){
			HINTERNET hOpen = 0;
			HINTERNET hConnect = 0;
			HINTERNET hRequest = 0;
			IStream *stream = NULL;
...
// 				}
		...
		std::string ReadHTTP(const wchar_t* domain,
			const wchar_t* path, const int port){
			HINTERNET hOpen = 0;
			HINTERNET hConnect = 0;
			HINTERNET hRequest = 0;
			IStream *stream = NULL;
			...
			for(;;){
				...
				hConnect = WinHttpConnect(hOpen, domain, port ? port : INTERNET_DEFAULT_PORT, 0);
				if(!hConnect){
					break;
				}
				const wchar_t* types[50] = { { L"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8" }, { 0 } };
				hRequest = WinHttpOpenRequest(hConnect, L"GET", path, NULL, WINHTTP_NO_REFERER, types, WINHTTP_FLAG_BYPASS_PROXY_CACHE);
				if(!hRequest){
					break;
				}
				std::wstring request_header = L"Accept-Language: zh-cn,zh;q=0.5\r\nConnection: Keep-Alive\r\nDNT: 1\r\n";
				if (!WinHttpSendRequest(hRequest, request_header.c_str(), request_header.length(), nullptr, 0, 0, WINHTTP_FLAG_BYPASS_PROXY_CACHE)){
					std::function<std::string(const wchar_t*, const wchar_t*, const int)> ReadSocket = [](const wchar_t* domain,
						const wchar_t* path, const int port)->std::string{
						...
						std::string headers;
						headers.append("GET ");
						headers.append(W2A(path));
						headers.append(" HTTP/1.1\r\n");
						headers.append("Host: ");
						headers.append(W2A(domain));
						headers.append(":");
						char port_buffer[256] = {0};
						itoa(port, port_buffer, 10);
						headers.append(port_buffer);
						headers.append("\r\n");
						...
						while ((nDataLength = recv(Socket, buffer, 10000, 0)) > 0){
							loc_message.append(buffer, nDataLength);
							memset(buffer, 0, 10000);
						}
						closesocket(Socket);
						if(loc_message.find("Transfer-Encoding: chunked")!=std::string::npos){
							int pos = loc_message.find("\r\n\r\n");
							if (pos != std::string::npos){
								loc_message = &loc_message[pos + 4];
							}
							pos = loc_message.find("\r\n");
							if (pos != std::string::npos){
								loc_message = &loc_message[pos + 2];
							}
							pos = loc_message.find("\r\n");
							if (pos != std::string::npos){
								loc_message[pos] = 0;
					...
					message = ReadSocket(domain, path, port);
 					break;
				}
				if(!WinHttpReceiveResponse(hRequest,0)){
					break;
